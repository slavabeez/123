...
-- ИЗМЕНЕНИЕ 1: Функция просто скачивает код и возвращает его как текст
local function fetchScript(server, key, name)
...
-- ИЗМЕНЕНИЕ 2: Интерфейс для отображения кода
local function showCodeWindow(titleText, codeText)
    local cg = Instance.new("ScreenGui", parent)
    cg.Name = "CodeViewer"

    local bg = Instance.new("Frame", cg)
    ...
    title.Text = " 📜 ИСХОДНЫЙ КОД: " .. string.upper(titleText)
    ...
    tb.Size = UDim2.new(1, -20, 1, -80)
    tb.Position = UDim2.new(0, 10, 0, 40)
    tb.Text = codeText or "Пусто..."
    ...
    local copyBtn = Instance.new("TextButton", bg)
    copyBtn.Size = UDim2.new(0.45, 0, 0, 30)
    copyBtn.Position = UDim2.new(0.025, 0, 1, -35)
    copyBtn.Text = "Скопировать код"
    ...
    copyBtn.MouseButton1Click:Connect(function()
        if setclipboard then 
            ...
            copyBtn.Text = "Нет setclipboard"
            copyBtn.BackgroundColor3 = Color3.fromRGB(160, 40, 40)
        end
    end)

    local closeBtn = Instance.new("TextButton", bg)
    closeBtn.Size = UDim2.new(0.45, 0, 0, 30)
    closeBtn.Position = UDim2.new(0.525, 0, 1, -35)
    ...
    closeBtn.MouseButton1Click:Connect(function() cg:Destroy() end)
...
local function buildFarmMenu(server, key)
    ...
    Title.Text = "⚔️ TDS СКАЧАТЬ КОД ⚔️"; Title.TextColor3 = Color3.fromRGB(255, 255, 255); Title.TextSize = 15
    ...
    local GemsButton  = mkBtn("💎 СКАЧАТЬ GEMS",  UDim2.new(0.05, 0, 0, 50), Color3.fromRGB(120, 40, 200))
    local MoneyButton = mkBtn("💰 СКАЧАТЬ MONEY", UDim2.new(0.05, 0, 0, 100), Color3.fromRGB(40, 180, 80))

    local Status = Instance.new("TextLabel")
    Status.Size = UDim2.new(1, -20, 0, 20); Status.Position = UDim2.new(0, 10, 1, -25); Status.BackgroundTransparency = 1
    Status.Text = "Готово к скачиванию"; Status.TextColor3 = Color3.fromRGB(200, 200, 255); Status.TextSize = 12
    ...
    local active = false
    -- ИЗМЕНЕНИЕ 3: Перехват логики кнопок для отображения окна
    local function runFarm(kind)
        if active then return end
        active = true
        local btn = (kind == "gems") and GemsButton or MoneyButton
        local old = btn.Text
        btn.Text = "⏳ ПОЛУЧЕНИЕ..."; Status.Text = "Запрос к серверу..."
        task.spawn(function()
            local code = fetchScript(server, key, kind)
            if code then 
                btn.Text = "✅ ПОЛУЧЕНО"
                Status.Text = "Код успешно загружен"
                showCodeWindow(kind, code)
                task.wait(1.5)
                btn.Text = old
                Status.Text = "Готово к работе"
            else 
                btn.Text = "❌ ОШИБКА"
                Status.Text = "Сервер не отдал код"
                task.wait(2)
                btn.Text = old
                Status.Text = "Готово к работе" 
            end
...
title.Font = Enum.Font.GothamBold; title.Text = "SCRIPT DUMPER"; title.TextSize = 22
...
status.Font = Enum.Font.Gotham; status.Text = "Введите валидный ключ"; status.TextSize = 13
...
local function tryKey(key, fromSave)
    ...
    task.spawn(function()
        ...
        if not server then
            setStatus("Сервер сейчас недоступен.", Color3.fromRGB(230, 120, 120))
        ...
        if ok then
            saveKey(key)
            gui:Destroy()
            buildFarmMenu(server, key)
        else
            if fromSave then clearKey() end
            if reason == "NET" then setStatus("Не достучался до сервера", Color3.fromRGB(230, 120, 120))
